...
The wheel force-includes ``src/leetha/ui/web/dist``, but that directory is
generated by ``bun run build`` and is git-ignored, so it does not exist in a
fresh clone. Hatchling raises ``FileNotFoundError`` on a missing
force-include, which made every git-based install fail:

    pipx install git+https://github.com/tjnull/leetha.git
    pip  install git+https://github.com/tjnull/leetha.git
    uv build

None of those can run the frontend build first, so they never got past
metadata generation.

This hook guarantees the directory exists before the force-include is
resolved. When a real build is present it is left untouched; otherwise a
placeholder page is written that explains how to produce the real UI. The
CLI, capture engine, and API are unaffected either way -- only the dashboard
is a placeholder until the frontend is built.
"""
